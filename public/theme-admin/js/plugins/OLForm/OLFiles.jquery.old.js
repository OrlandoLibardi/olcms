(function($) {
	...
	$.fn.OLFiles = function(options, callback) {
			...
			function init() {
				...
				console.log("OLFiles initialize");
				checkValues();
		...
		/*modelo list files*/
			function initListFiles(files){
						...
						$.each(files, function(a, b){
											obj += '<div class="file-box">';
											obj += '<div class="file">';
											obj += '<a href="javascript:;" data-folder="'+b.realname+'" data-extension="'+b.size+'" data-url="'+b.url+'">';
											obj += '<div class="image">';
		...
		function initFromFolder(directory){
		 ...
		 /*modelo list breadcrumbs*/
			...
			function getFilesAndFolders(folder){
					console.log("OLFROM SENDDING...", folder);
					...
					$.ajax({
						...
						beforeSend: function() {
							...
							console.log("get files loadding...");
						},
						success: function(exr) {
							...
							closeAlert();
							console.log("get files success...", exr);	
						},
						error: function(exr, sender) {
							/*
							showErros(exr);*/
							showAlert(exr.responseJSON.message, exr.responseJSON.status);
							console.log("get files error...", exr);
						},
						complete: function() {
		...
		function initFormFiles(){
							 var obj = '<form action="'+settings.actionSendFile+'" '; 
							                ...
							                obj += 'enctype="multipart/form-data">';
																								 //obj += '<input type="hidden" name="directory" value="ola mundo">';					
		...
		function showAlert(msg, status) {
		 ...
		 /*CHECK VALUES*/
			function checkValues() {
				/*ACTIONS*/
			if (!settings.actionListFolders) {				
					throw new Error('OLFiles: ACTION LIST FOLDER não encontrada!');
			}
			if (!settings.actionCreateFolder) {				
					throw new Error('OLFiles: ACTION CREATE FOLDER não encontrada!');
			}
			if (!settings.actionOpenFile) {				
					throw new Error('OLFiles: ACTION OPEN FILE não encontrada!');
			}
			if (!settings.actionSendFile) {				
					throw new Error('OLFiles: ACTION SEND FILE não encontrada!');
			}
			if(!settings.initialFolder){
					throw new Error('OLFiles: INITIAL FOLDER não configurada!');
				}
			/*AREAS*/
			if(!$(settings.areaDrop)){
				throw new Error('OLFiles: DROP AREA não encontrada!');
			}	
			if(!$(settings.areaListFiles)){
				throw new Error('OLFiles: LIST FILES AREA não encontrada!');
			}	
			if(!$(settings.areaListDirs)){
				throw new Error('OLFiles: LIST DIRECTORIES AREA não encontrada!');
			}
			if(!$(settings.areaFormFolder)){
				throw new Error('OLFiles: FORM FOLDER AREA não encontrada!');
			}
		...
		function findToken() {
			...
			if (!token) {
				throw new Error('OLFiles: TOKEN e ou CSRF-TOKEN não encontrados!');
			}
			...
			$(settings.areaDrop).addClass('active');
					...
					$.ajax({
								 ...
								 xhr : function ()
									...
									success: function(exr) {
												console.log("response", exr.message);
												//showAlert(exr.message, 'success');
									...
									error: function(exr) {
													$(settings.areaDrop).removeClass('active');
													$input.val("");
													console.log("ERROR ", exr.responseJSON.message);
													console.log("ERROS ", exr.responseJSON.errors);
